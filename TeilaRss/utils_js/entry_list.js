...
//  listitem.onclick = function() { openEntryInBrowser(item.link); };
  listitem.ondblclick = function() {
    debug.trace('titl : ' + item.title);
    debug.trace('desc : ' + item.content);
    debug.trace('link : ' + item.link);
    openEntryInBrowser(item.link);
  };
...
//  this.title.height = headerHeight;
  ...
  this.entryListViewport.height = this.contentHeight ;
  ...
  if (this.entryListViewport.height > this.entryList.height) {
	...
	this.scrollProgressBar.value = 90;
  } else {
